...
struct SimulationParams {
  ...
  float boundary_margin = 0.2;      // fraction of extent where boundary force engages
  float boundary_stiffness = 6.0;   // strength of boundary penalty
};
...
copy_params(const T* params, T** d_out)
{
  if ( !params || !d_out ) {
    std::cerr << "[ERROR] nullptr passed to copy_params\n";
    return cudaErrorInvalidValue; 
  }

  T* d_params = nullptr; 
  cudaError_t status = cudaMalloc(&d_params, sizeof(T));
  if ( status != cudaSuccess ) {
    return status; 
  }

  if ( (status = cudaMemcpy(d_params, params, 
          sizeof(T), cudaMemcpyHostToDevice)) != cudaSuccess ) {
    cudaFree(d_params);
    return status; 
  }
